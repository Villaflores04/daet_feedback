...
export function MoodDonut({ positive, mixed, negative, size = 160, className = "", showCenter = false, active, onSelect }: {
  ...
  return <div className={`mood-donut ${className}`} style={{width:size,height:size}}>
    <svg viewBox="0 0 120 120" role="img" aria-label={`${totalPulses} pulses: ${positive} positive, ${mixed} mixed, ${negative} needs care`}>
      ...
      {segs.map(s=>{
        const fraction=s.value/total; const dash=Math.max(0,fraction*c-gap);
        const isActive=!active||active===s.key;
        const select=()=>onSelect?.(s.key);
        return <circle key={s.key} className={`mood-donut-seg ${isActive?"":"dim"}`} cx="60" cy="60" r={r} fill="none"
          stroke={s.color} strokeWidth={active===s.key?15:12} strokeLinecap="round"
          strokeDasharray={`${dash} ${c-dash}`} strokeDashoffset={-offset} pathLength={c}
          transform="rotate(-90 60 60)" tabIndex={onSelect?0:undefined} role={onSelect?"button":undefined}
          aria-label={onSelect?`Show ${s.key} places`:undefined}
          onClick={onSelect?select:undefined}
          onKeyDown={e=>{if(onSelect&&(e.key==="Enter"||e.key===" ")){e.preventDefault();select();}}}/>;
      }).map((circle,i)=>{ offset += (segs[i].value/total)*c; return circle; })}
    </svg>
